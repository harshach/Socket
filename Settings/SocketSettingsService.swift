...
private struct SocketSettingsServiceKey: EnvironmentKey {
    @MainActor
    static var defaultValue: SocketSettingsService {
        // This should never be called since we always inject from SocketApp
        // But EnvironmentKey protocol requires a default value
        return SocketSettingsService()
    }
}

extension EnvironmentValues {
    var socketSettings: SocketSettingsService {
        get { self[SocketSettingsServiceKey.self] }
        set { self[SocketSettingsServiceKey.self] = newValue }
    }
}
...
public enum TabLayout: String, CaseIterable, Identifiable {
    ...
    public static var allCases: [TabLayout] { [.sidebar] }
