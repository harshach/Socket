...
struct SocketButtonStyle: ButtonStyle {
...
extension ButtonStyle where Self == SocketButtonStyle {
    static var socketButton: SocketButtonStyle {
        SocketButtonStyle(variant: .secondary, shadowStyle: .subtle, role: nil)
    }

    static func socketButton(role: ButtonRole?) -> SocketButtonStyle {
        SocketButtonStyle(variant: .secondary, shadowStyle: .subtle, role: role)
    }

    static var socketButtonProminent: SocketButtonStyle {
        SocketButtonStyle(variant: .primary, shadowStyle: .prominent, role: nil)
    }

    static func socketButtonProminent(role: ButtonRole?) -> SocketButtonStyle {
        SocketButtonStyle(variant: .primary, shadowStyle: .prominent, role: role)
    }
}
...
private struct ButtonPreviewSection: View {
    ...
    private var buttonStack: some View {
        VStack(spacing: 20) {
            ...
            .buttonStyle(.socketButtonProminent)
            .background(.red)
            ...
            .buttonStyle(.socketButton(role: .destructive))

            Button("Erase Everything", systemImage: "flame") {
                print("Erase")
            }
            .buttonStyle(.socketButtonProminent(role: .destructive))
