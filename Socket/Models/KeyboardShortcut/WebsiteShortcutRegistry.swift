...
//  When Socket and a website share the same shortcut, we enable a "double-press"
//  system where the first press goes to the website, and the second press
//  (within 1 second) goes to Socket.
//
...
/// Represents a website/webapp and its keyboard shortcuts that may conflict with Socket.
...
struct WebsiteShortcut: Codable, Hashable {
    ...
    /// Create lookup key for matching with Socket shortcuts
...
extension WebsiteShortcutProfile {
    ...
    static let knownProfiles: [WebsiteShortcutProfile] = [
        ...
        .init(
            ...
            shortcuts: [
                // Note: Gmail uses many single-key shortcuts, but most don't conflict with Socket
                // since Socket requires modifiers for most actions. We only track conflicts.
        ...
        .init(
            ...
            shortcuts: [
                // YouTube uses single-key shortcuts that don't usually conflict with Socket
                // since Socket requires modifiers. Including for completeness.
        ...
        .init(
            ...
            shortcuts: [
                // Single-key shortcuts, low conflict with Socket
...
struct ShortcutConflictInfo: Equatable {
    ...
    let socketActionName: String
    let timestamp: Date
    let windowId: UUID
    
    init(keyCombination: KeyCombination, websiteName: String, websiteShortcutDescription: String?, socketActionName: String, windowId: UUID) {
        self.keyCombination = keyCombination
        self.websiteName = websiteName
        self.websiteShortcutDescription = websiteShortcutDescription
        self.socketActionName = socketActionName
        self.timestamp = Date()
