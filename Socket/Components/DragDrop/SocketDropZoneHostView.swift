...
@MainActor
class SocketDropZoneCoordinator: NSObject {
    var zoneID: DropZoneID
    var isVertical: Bool
    let manager: SocketDragSessionManager

    init(zoneID: DropZoneID, isVertical: Bool, manager: SocketDragSessionManager) {
        self.zoneID = zoneID
...
// MARK: - SocketDropZoneNSView

class SocketDropZoneNSView: NSView {
    weak var coordinator: SocketDropZoneCoordinator?
...
private struct DropZoneAnchor: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> SocketDropZoneNSView {
        let view = SocketDropZoneNSView()
        view.coordinator = context.coordinator
        view.registerForDraggedTypes([.socketTabItem, .string])
        return view
    }

    func updateNSView(_ nsView: SocketDropZoneNSView, context: Context) {
        context.coordinator.zoneID = zoneID
        context.coordinator.isVertical = isVertical
    }

    func makeCoordinator() -> SocketDropZoneCoordinator {
        SocketDropZoneCoordinator(zoneID: zoneID, isVertical: isVertical, manager: manager)
    }
}

// MARK: - SocketDropZoneHostView

struct SocketDropZoneHostView<Content: View>: View {
    let zoneID: DropZoneID
    let isVertical: Bool
    let manager: SocketDragSessionManager
