//
//  SocketDragSessionManager.swift
//  Socket
//
//
...
@MainActor
final class SocketDragSessionManager: ObservableObject {
    static let shared = SocketDragSessionManager()

    @Published var draggedItem: SocketDragItem?
    ...
    private var previewWindow: SocketDragPreviewWindow?

    private func ensurePreviewWindow() {
        guard previewWindow == nil else { return }
        previewWindow = SocketDragPreviewWindow(manager: self)
    }

    private struct WeakDragSource {
        weak var view: SocketDragSourceNSView?
    }
    ...
    func registerDragSource(_ view: SocketDragSourceNSView, id: UUID) {
    ...
    func beginDrag(item: SocketDragItem, tab: Tab, from zone: DropZoneID, at index: Int) {
        #if DEBUG
    ...
    private func _updateCursorScreenPosition(_ screenPoint: NSPoint) {
        ...
        guard let window = NSApp.mainWindow ?? NSApp.windows.first(where: { $0.isVisible && !($0 is SocketDragPreviewWindow) }),
