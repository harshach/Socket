...
// MARK: - SocketDragPreviewWindow

class SocketDragPreviewWindow: NSWindow {
    static let previewSize = NSSize(width: 320, height: 160)

    private var hostingView: NSHostingView<AnyView>?
    private var cancellables = Set<AnyCancellable>()
    private weak var manager: SocketDragSessionManager?

    @MainActor
    init(manager: SocketDragSessionManager) {
        ...
        let content = SocketDragPreviewContent(manager: manager)
    ...
    @MainActor
    private func observeManager(_ manager: SocketDragSessionManager) {
        manager.$draggedItem
            .map { $0 != nil }
            .removeDuplicates()
...
private enum SocketPreviewStyle: Equatable {
    case tabRow
    case pinnedTile
    case ghost
...
private struct SocketDragPreviewContent: View {
    @ObservedObject var manager: SocketDragSessionManager
    ...
    var body: some View {
        ...
        .frame(width: SocketDragPreviewWindow.previewSize.width, height: SocketDragPreviewWindow.previewSize.height)
    }

    private var currentStyle: SocketPreviewStyle {
...
private struct SocketMorphingPreview: View {
    let tab: Tab?
    let title: String
    let style: SocketPreviewStyle
