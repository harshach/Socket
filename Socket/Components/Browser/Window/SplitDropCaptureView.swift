...
final class SplitDropCaptureView: NSView {
    ...
    private func commonInit() {
        ...
        registerForDraggedTypes([.socketTabItem, .string])
    ...
    override func performDragOperation(_ sender: NSDraggingInfo) -> Bool {
        ...
        // Try to read SocketDragItem first, fall back to plain string UUID
        let tabId: UUID? = {
            if let item = SocketDragItem.fromPasteboard(pb) { return item.tabId }
        ...
        DispatchQueue.main.async {
            SocketDragSessionManager.shared.cancelDrag()
        }
        isDragActive = false
        return true
    }
