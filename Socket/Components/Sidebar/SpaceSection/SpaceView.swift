...
struct SpaceView: View {
    ...
    private var regularChildTabs: [UUID: [Tab]] {
        Dictionary(grouping: tabs.filter { $0.parentTabId != nil }) { $0.parentTabId! }
    }

    private var regularDisplayRows: [(tab: Tab, depth: Int)] {
        if windowState.isIncognito {
            return windowState.ephemeralTabs.sorted { $0.index < $1.index }.map { ($0, 0) }
        }
        return browserManager.tabManager.displayOrderedTabsWithDepth(in: space)
    }
    ...
    private func pinnedTabView(_ tab: Tab, index: Int) -> some View {
        SocketDragSourceView(
            item: SocketDragItem(tabId: tab.id, title: tab.name, urlString: tab.url.absoluteString),
            tab: tab,
            zoneID: .spacePinned(space.id),
            index: index,
            manager: dragSession
    ...
    private func regularTabsView(currentTabs: [Tab]) -> some View {
        VStack(spacing: 2) {
            ForEach(Array(regularDisplayRows.enumerated()), id: \.element.tab.id) { fallbackIndex, row in
                let index = currentTabs.firstIndex(where: { $0.id == row.tab.id }) ?? fallbackIndex
                regularTabView(
                    row.tab,
                    index: index,
                    depth: row.depth,
                    hasChildren: !(regularChildTabs[row.tab.id] ?? []).isEmpty
                )
            }
        }
    }
    ...
    private func regularTabView(_ tab: Tab, index: Int, depth: Int = 0, hasChildren: Bool = false) -> some View {
        SocketDragSourceView(
            item: SocketDragItem(tabId: tab.id, title: tab.name, urlString: tab.url.absoluteString),
            tab: tab,
            zoneID: .spaceRegular(space.id),
