...
struct SpaceTab: View {
    ...
    private var hierarchyTint: Color {
        isCurrentTab ? AppColors.textPrimary : AppColors.textSecondary
    }
            ...
            HStack(spacing: 8) {
                ...
                if hasChildren {
                    Image(systemName: "chevron.down")
                        ...
                        .foregroundStyle(hierarchyTint.opacity(0.72))
                        .frame(width: 10)
                } else if depth > 0 {
                    Image(systemName: "arrow.turn.down.right")
                        .font(.system(size: 9, weight: .semibold))
                        .foregroundStyle(hierarchyTint.opacity(0.5))
                        .frame(width: 10)
                }
    ...
    private var closeButtonSlot: some View {
        Button(action: onClose) {
            Image(systemName: "xmark")
                ...
                .background(
                    isCloseHovering
                        ? (isCurrentTab ? AppColors.controlBackgroundHoverLight : AppColors.controlBackgroundActive)
                        : Color.clear
                )
