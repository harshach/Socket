...
struct PinnedGrid: View {
    ...
    var body: some View {
        let pinnedTabsConfiguration: PinnedTabsConfiguration = socketSettings.pinnedTabsLook
                        ...
                        LazyVGrid(columns: columns, alignment: .center, spacing: pinnedTabsConfiguration.gridSpacing) {
                            ...
                            ForEach(Array(items.enumerated()), id: \.element.id) { index, tab in
                                ...
                                SocketDragSourceView(
                                    item: SocketDragItem(tabId: tab.id, title: title, urlString: tab.url.absoluteString),
                                    tab: tab,
                                    zoneID: .essentials,
                                    index: index,
                                    manager: dragSession
    ...
    private var essentialsPlaceholder: some View {
        RoundedRectangle(cornerRadius: 12, style: .continuous)
            ...
            .frame(minWidth: socketSettings.pinnedTabsLook.minWidth, minHeight: socketSettings.pinnedTabsLook.minWidth)
    }
    ...
    private func columnCount(for width: CGFloat, itemCount: Int) -> Int {
        ...
        var cols = min(socketSettings.pinnedTabsLook.maxColumns, itemCount)
        while cols > 1 {
            let needed = CGFloat(cols) * socketSettings.pinnedTabsLook.minWidth + CGFloat(cols - 1) * socketSettings.pinnedTabsLook.gridSpacing
    ...
    private func makeColumns(count: Int) -> [GridItem] {
        Array(
            repeating: GridItem(
                .flexible(minimum: socketSettings.pinnedTabsLook.minWidth),
                spacing: socketSettings.pinnedTabsLook.gridSpacing,
                alignment: .center
            ),
            count: count
        )
