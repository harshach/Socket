...
struct MiniWindowWebView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler {
        ...
        private func writeAuthTrace(_ message: String) {
            let formatter = ISO8601DateFormatter()
            let timestamp = formatter.string(from: Date())
            let line = "[\(timestamp)] 🔐 [MiniWindowTrace] \(message)\n"
            let logDir = FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent("Library/Logs/Socket", isDirectory: true)
            let logURL = logDir.appendingPathComponent("auth-trace.log")
            ...
            do {
                ...
                print("❌ [MiniWindow] Failed to write auth trace: \(error.localizedDescription)")
            }
        }
        ...
        func loadInitialURLIfNeeded(on webView: WKWebView) {
            ...
            writeAuthTrace(
                "loadInitialURLIfNeeded url=\(session.currentURL.absoluteString) profile=\(session.profile?.id.uuidString ?? "nil") store=\(webView.configuration.websiteDataStore.identifier?.uuidString ?? "default")"
            )
...
extension MiniWindowWebView.Coordinator: WKNavigationDelegate {
    func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        ...
        writeAuthTrace("didStart url=\(webView.url?.absoluteString ?? "nil")")
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        writeAuthTrace("didFinish url=\(webView.url?.absoluteString ?? "nil") title=\(webView.title ?? "")")
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        ...
        writeAuthTrace("didFail url=\(webView.url?.absoluteString ?? "nil") error=\(error.localizedDescription)")
        session.updateLoading(isLoading: false)
    }

    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        print("🔐 [MiniWindow] Provisional navigation failed: \(error.localizedDescription)")
        writeAuthTrace("didFailProvisional url=\(webView.url?.absoluteString ?? "nil") error=\(error.localizedDescription)")
        session.updateLoading(isLoading: false)
    }
}
...
extension MiniWindowWebView.Coordinator: WKUIDelegate {
    func webView(
        ...
        let popupURL = navigationAction.request.url?.absoluteString ?? "nil"
        let sourceURL = navigationAction.sourceFrame.request.url?.absoluteString ?? "nil"
        writeAuthTrace(
            "createWebViewWith popupURL=\(popupURL) sourceURL=\(sourceURL) targetFrame=\(navigationAction.targetFrame == nil ? "nil" : "existing") width=\(windowFeatures.width?.stringValue ?? "nil") height=\(windowFeatures.height?.stringValue ?? "nil")"
        )
