...
//  Socket and a website. Informs users they can press again for Socket action.
//
        ...
        ToastView {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    // Subtitle: press again for Socket
                    HStack(spacing: 4) {
                        Text("Press again for")
                            .font(.system(size: 11))
                            .foregroundStyle(.white.opacity(0.7))
                        Text(conflictInfo.socketActionName)
...
#Preview {
    ShortcutConflictToast(
        conflictInfo: ShortcutConflictInfo(
            ...
            socketActionName: "Command Palette",
            windowId: UUID()
        )
    )
    .padding()
