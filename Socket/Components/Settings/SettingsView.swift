...
struct SettingsView: View {
    ...
    var body: some View {
        SettingsContent(socketSettings: socketSettings, browserManager: browserManager, gradientColorManager: gradientColorManager)
    }
}

private struct SettingsContent: View {
    @Bindable var socketSettings: SocketSettingsService
    @ObservedObject var browserManager: BrowserManager
    @ObservedObject var gradientColorManager: GradientColorManager

    var body: some View {
        TabView(selection: $socketSettings.currentSettingsTab) {
            SettingsPane {
                SettingsGeneralTab()
            }
            .tabItem {
        ...
        HStack(alignment: .top, spacing: 16) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    SettingsSectionCard(
                        title: "Socket",
                        subtitle: "General Socket settings"
                    ) {
                        VStack(alignment: .leading, spacing: 16) {
                            Toggle(
                                isOn: $settings
                    ...
                    SettingsSectionCard(
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            if socketSettings.showAIAssistant {
                                ...
                                Text(socketSettings.geminiModel.description)
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                        }
                        ...
                        VStack(spacing: 8) {
                            ForEach(
                                ...
                                ProfileRowView(
                                    ...
                                    onChangeAppearance: {
                                        browserManager.showProfileAppearanceEditor(for: profile)
                                    },
                                    onManageData: {
        ...
        browserManager.dialogManager.showDialog(
            ProfileCreationDialog(
                ...
                onCreate: { name, icon in
                    _ = browserManager.createProfileAndActivate(
                        name: name,
                        icon: icon
                    )
...
struct ShortcutsSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Sigma Command Mode")
                        .font(.subheadline)
                        .fontWeight(.medium)
                    Text("Enable modifier-less shortcuts like J/K/D, Space, and / when you are not typing into a field.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                ...
                Spacer()
                Toggle("", isOn: Binding(
                    get: { socketSettings.sigmaCommandModeEnabled },
                    set: { socketSettings.sigmaCommandModeEnabled = $0 }
                ))
                    .toggleStyle(.switch)
                    .controlSize(.small)
                    .accessibilityLabel("Sigma Command Mode")
            }
            .padding(12)
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Detect Website Shortcuts")
                        .font(.subheadline)
                        .fontWeight(.medium)
                    Text("When a website uses the same shortcut, press once for website, twice for Socket")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
...
private struct ShortcutRowView: View {
        ...
        HStack(spacing: 12) {
            ...
            if shortcut.isCustomizable {
                Toggle("", isOn: Binding(
                    ...
                    set: { newValue in
                        keyboardShortcutManager.toggleShortcut(shortcutId: shortcut.id, isEnabled: newValue)
                    }
                ))
                .toggleStyle(.switch)
                .labelsHidden()
    ...
    private func updateShortcut() {
        keyboardShortcutManager.updateShortcut(shortcutId: shortcut.id, keyCombination: localKeyCombination)
    }
}
    ...
    var body: some View {
        ...
        .sheet(item: $editingEntry) { entry in
            SiteSearchEntryEditor(entry: entry) { updated in
                if let idx = socketSettings.siteSearchEntries.firstIndex(where: { $0.id == updated.id }) {
                    socketSettings.siteSearchEntries[idx] = updated
                }
            }
        }
    }
