...
class KeyboardShortcutManager {
    ...
    private func mergeWithDefaults() {
        ...
        for defaultShortcut in defaultShortcuts {
            let exists = shortcutMap.values.contains {
                $0.action == defaultShortcut.action && $0.lookupKey == defaultShortcut.lookupKey
            }
            if !exists {
    ...
    func updateShortcut(shortcutId: UUID, keyCombination: KeyCombination) {
        // Find the existing shortcut first
        guard var shortcut = shortcutMap.values.first(where: { $0.id == shortcutId }) else {
            return
        }
    ...
    func toggleShortcut(shortcutId: UUID, isEnabled: Bool) {
        if let key = shortcutMap.first(where: { $0.value.id == shortcutId })?.key,
    ...
    func hasConflict(keyCombination: KeyCombination, excludingShortcutId: UUID? = nil) -> ShortcutAction? {
        guard let shortcut = shortcutMap[keyCombination.lookupKey],
              shortcut.isEnabled else {
            return nil
        }

        if shortcut.id != excludingShortcutId {
    ...
    func isValidKeyCombination(_ keyCombination: KeyCombination) -> Bool {
        ...
        // Allow bare printable keys for Sigma-style command mode.
        if keyCombination.modifiers.isEmpty && keyCombination.key.count == 1 {
            return true
        }

        // Function and navigation keys remain valid with or without modifiers.
    ...
    private var sigmaCommandModeEnabled: Bool {
        browserManager?.socketSettings?.sigmaCommandModeEnabled ?? true
    }
    ...
    func executeShortcut(_ event: NSEvent) -> Bool {
        ...
        // UNLESS they have modifiers (cmd, option, etc.) - those might be Socket shortcuts
        ...
        if navigationKeyCodes.contains(keyCode) {
            ...
            let hasModifiers = event.modifierFlags.contains(.command) ||
                              ...
                              event.modifierFlags.contains(.control) ||
                              event.modifierFlags.contains(.shift)
            let sigmaArrowShortcutCodes: Set<UInt16> = [125, 126] // down/up arrows
            if !hasModifiers && !sigmaArrowShortcutCodes.contains(keyCode) {
                return false // Pass through for pure navigation (no modifiers)
            }
            // Has modifiers - continue to check if it's a Socket shortcut
        }
        ...
        if !shouldHandleModifierlessShortcut(keyCombination) {
            print("⌨️ [KSM] Skipping modifierless shortcut due to typing context or disabled command mode")
            return false
        }

        guard let shortcut = shortcutMap[keyCombination.lookupKey],
              shortcut.isEnabled else {
            print("⌨️ [KSM] No Socket shortcut for: \(keyCombination.lookupKey)")
            return false
        }
        
        print("⌨️ [KSM] Found Socket shortcut: \(shortcut.action.displayName)")
        ...
        if let windowId = windowRegistry?.activeWindow?.id {
            ...
            let shouldPass = websiteShortcutDetector.shouldPassToWebsite(
                keyCombination,
                windowId: windowId,
                socketActionName: shortcut.action.displayName
            )
            ...
            if shouldPass {
                ...
                // Second press within timeout - we'll execute Socket action below
                print("⌨️ [KSM] >>> SECOND PRESS - Executing Socket action <<<")
            } else {
                print("⌨️ [KSM] No conflict detected, executing Socket action")
            }
        } else {
            print("⌨️ [KSM] No active window found for conflict check")
        }

        print("⌨️ [KSM] Executing Socket shortcut: \(shortcut.action.displayName)")
    ...
    private func executeAction(_ action: ShortcutAction) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            case .moveCurrentPageUp:
                if let currentTabId = browserManager.currentTabForActiveWindow()?.id {
                    browserManager.tabManager.moveTabUp(currentTabId)
                }
            case .moveCurrentPageDown:
                if let currentTabId = browserManager.currentTabForActiveWindow()?.id {
                    browserManager.tabManager.moveTabDown(currentTabId)
                }
            ...
            case .goToWorkspace1, .goToWorkspace2, .goToWorkspace3, .goToWorkspace4, .goToWorkspace5, .goToWorkspace6, .goToWorkspace7, .goToWorkspace8, .goToWorkspace9:
                let workspaceIndex = Int(action.rawValue.components(separatedBy: "_").last ?? "1") ?? 1
                browserManager.selectSpaceByIndexInActiveWindow(workspaceIndex - 1)
            case .createWorkspace:
                browserManager.showCreateSpaceDialog()
            case .editCurrentWorkspace:
                browserManager.showEditCurrentSpaceDialog()
            case .moveWorkspaceUp:
                browserManager.moveCurrentSpaceUp()
            case .moveWorkspaceDown:
                browserManager.moveCurrentSpaceDown()
            ...
            case .toggleFocusMode:
                browserManager.toggleFocusMode()
            case .enterInsertMode:
                browserManager.setInsertModeEnabled(true)
            case .focusOutOfWebContent:
                browserManager.focusOutOfWebContent()
            case .focusSplitScreen:
                browserManager.focusSplitScreen()
            case .toggleSplitScreenFocus:
                browserManager.toggleSplitScreenFocus()
            case .toggleSplitMode:
                browserManager.toggleSplitMode()
            case .sendCurrentPageToSplit:
                browserManager.sendCurrentPageToSplit()
            case .sendSplitPageToMain:
                browserManager.promoteSplitPageToMain()
            case .closeSplitPage:
                browserManager.closeSplitPage()
            case .refreshSplitPage:
                browserManager.refreshSplitPage()

            // Tools & Features
            case .openCommandPalette:
                self.windowRegistry?.activeWindow?.commandPalette?.open()
            case .openCommandPaletteInSplit:
                self.windowRegistry?.activeWindow?.commandPalette?.openInSplit()
            case .replaceCurrentPageWithPalette:
            ...
            case .openExtensionsPanel:
                browserManager.showExtensionsSettings()

            // NEW: Missing actions that were only in SocketCommands
