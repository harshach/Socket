    ...
    WKWebExtensionControllerDelegate, NSPopoverDelegate
{
    ...
    private func setupExtensionController() {
        ...
        if let idString = UserDefaults.standard.string(
            forKey: "Socket.WKWebExtensionController.Identifier"
        ),
    ...
    private func setupExternallyConnectableBridge(
        ...
        // pages and relays extension traffic via window.postMessage to socket_bridge.js (ISOLATED world).
        let polyfillJS = """
        (function() {
            ...
            if (window.__socketEcShimInstalled) return;
            window.__socketEcShimInstalled = true;

            console.log('[SOCKET-EC] Installing externally_connectable polyfill on ' + location.href);
                    ...
                    dispatch: function() {
                        ...
                        for (var i = 0; i < snapshot.length; i += 1) {
                            try {
                                ...
                                console.error('[SOCKET-EC] Listener error:', error);
                            }
                        }
                    }
                };
            ...
            function adoptBridgeRuntime(runtimeId) {
                ...
                _bridgeRuntimeRetargetTimer = setTimeout(function() {
                    ...
                    console.warn('[SOCKET-EC] Bridge runtime mismatch; retargeting to ' + _activeRuntimeId + ' (configured ' + _configuredRuntimeId + ')');
                    markBridgeReady();
                }, 250);
            }
            ...
            function requestViaBridge(parsed, requestType) {
                ...
                if (dedupKey) {
                    ...
                    if (inFlight && typeof inFlight.then === 'function') {
                        console.log('[SOCKET-EC] Reusing in-flight request key=' + dedupKey);
                        return inFlight;
                    }
                }
                ...
                var promise = waitForBridgeReady(3000).then(function() {
                    return new Promise(function(resolve, reject) {
                        var callbackId = 'socket_ec_' + Math.random().toString(36).slice(2, 11);
                        var timeoutId = null;
                        var settled = false;
                        ...
                        function settleSuccess(response) {
                            ...
                            if (normalizedType === 'fork' && response && typeof response === 'object') {
                                console.log('[SOCKET-EC] fork response ok=' + String(response.ok) + ' ext=' + (parsed.extensionId || '(none)'));
                            }
                            resolve(response);
                        }
                        ...
                        function handler(event) {
                            ...
                            if (!event.data || event.data.type !== 'socket_ec_response') return;
                            if (!matchesActiveRuntimeId(event.data.targetRuntimeId)) return;
                            if (event.data.callbackId !== callbackId) return;

                            if (event.data.error) {
                                console.warn('[SOCKET-EC] Response error for type=' + normalizedType + ': ' + event.data.error);
                                settleError(new Error(event.data.error));
                            } else {
                                console.log('[SOCKET-EC] Response success for type=' + normalizedType);
                        ...
                        timeoutId = setTimeout(function() {
                            console.warn('[SOCKET-EC] Timeout waiting for response type=' + normalizedType);
                            settleError(new Error('Extension communication timeout'));
                        }, 30000);

                        console.log('[SOCKET-EC] Forwarding request type=' + normalizedType + ' ext=' + (parsed.extensionId || '(none)'));
                        window.postMessage({
                            type: 'socket_ec_request',
            ...
            function createBridgePort(parsed) {
                var portId = 'socket_ec_port_' + Math.random().toString(36).slice(2, 11);
                ...
                waitForBridgeReady(3000).then(function() {
                    console.log('[SOCKET-EC] Opening bridge port id=' + portId + ' name=' + port.name + ' ext=' + (parsed.extensionId || '(none)'));
                    window.postMessage({
                        type: 'socket_ec_connect_open',
                        targetRuntimeId: activeRuntimeId(),
                        extensionId: parsed.extensionId,
                        connectInfo: connectInfo,
                        portId: portId
                    ...
                    console.warn('[SOCKET-EC] Bridge port open failed id=' + portId + ': ' + message);
            ...
            function makeSendMessageWrapper(originalSendMessage, runtimeKind, runtimeObject) {
                return function() {
                    ...
                    console.log('[SOCKET-EC] sendMessage called via ' + runtimeKind + ' type=' + requestType + ' ext=' + (parsed.extensionId || '(none)') + ' mode=' + (shouldBridge ? 'bridge' : 'native'));
                    ...
                    if (shouldBridge) {
                        ...
                        promise = promise.catch(function(error) {
                            var message = (error && error.message) ? error.message : String(error);
                            console.warn('[SOCKET-EC] Native sendMessage failed via ' + runtimeKind + ', falling back to bridge: ' + message);
            ...
            function makeConnectWrapper(originalConnect, runtimeKind, runtimeObject) {
                return function() {
                    ...
                    console.log('[SOCKET-EC] connect called via ' + runtimeKind + ' ext=' + (parsed.extensionId || '(none)') + ' mode=' + (shouldBridge ? 'bridge' : 'native'));
                    ...
                    try {
                        ...
                        var message = (error && error.message) ? error.message : String(error);
                        console.warn('[SOCKET-EC] Native connect failed via ' + runtimeKind + ', falling back to bridge: ' + message);
                        return createBridgePort(parsed);
                    }
                };
            }
            ...
            function installRuntimeShim(runtimeObject, runtimeKind) {
                ...
                var currentSendMessage = typeof runtimeObject.sendMessage === 'function'
                    ...
                    : null;
                if (runtimeObject.sendMessage !== runtimeObject.__socketEcWrappedSendMessage) {
                    runtimeObject.__socketEcWrappedSendMessage = makeSendMessageWrapper(
                        currentSendMessage,
                        runtimeKind,
                        runtimeObject
                    );
                    runtimeObject.sendMessage = runtimeObject.__socketEcWrappedSendMessage;
                }

                var currentConnect = typeof runtimeObject.connect === 'function'
                    ? runtimeObject.connect
                    : null;
                if (runtimeObject.connect !== runtimeObject.__socketEcWrappedConnect) {
                    runtimeObject.__socketEcWrappedConnect = makeConnectWrapper(
                        currentConnect,
                        runtimeKind,
                        runtimeObject
                    );
                    runtimeObject.connect = runtimeObject.__socketEcWrappedConnect;
                }
            }
            ...
            window.addEventListener('message', function(event) {
                ...
                if (event.data.type === 'socket_ec_bridge_ready') {
                    adoptBridgeRuntime(event.data.targetRuntimeId);
                    return;
                }
                if (event.data.type === 'socket_ec_connect_message') {
                    if (!matchesActiveRuntimeId(event.data.targetRuntimeId)) return;
                ...
                if (event.data.type === 'socket_ec_connect_disconnect') {
                    if (!matchesActiveRuntimeId(event.data.targetRuntimeId)) return;
            ...
            console.log('[SOCKET-EC] Polyfill ready — runtime sendMessage/connect wrapped (configured=' + _configuredRuntimeId + ')');
        })();
        """
        ...
        let retainedScripts = sharedUserContentController.userScripts.filter { script in
            ...
            guard source.contains("[SOCKET-EC] Installing externally_connectable polyfill on ") else {
                return true
            }
    ...
    private func patchManifestForWebKit(at manifestURL: URL) {
        ...
        if var contentScripts = manifest["content_scripts"] as? [[String: Any]] {
            for i in contentScripts.indices {
                ...
                if jsFiles.contains("socket_bridge.js") { continue }
        ...
        if let ec = manifest["externally_connectable"] as? [String: Any],
           let matchPatterns = ec["matches"] as? [String], !matchPatterns.isEmpty {
            ...
            let existingBridgeIndex = contentScripts.firstIndex { entry in
                (entry["js"] as? [String])?.contains("socket_bridge.js") == true
            }
            ...
            if let bridgeIndex = existingBridgeIndex {
                ...
                if currentAllFrames != true || currentRunAt != "document_start" || currentMatches != matchPatterns {
                    ...
                    manifest["content_scripts"] = contentScripts
                    changed = true
                    Self.logger.info("Updated existing socket_bridge.js content script entry for all-frames document_start coverage")
                }
            } else {
            ...
            let bridgeFileURL = bridgeDir.appendingPathComponent("socket_bridge.js")
            let bridgeJS = """
            // Socket: externally_connectable bridge relay
            ...
            (function() {
                ...
                function relaySendMessage(data) {
                    ...
                    console.log('[SOCKET-EC] Relay send type=' + outgoingType + ' version=' + (
                    ...
                    Promise.resolve(sendPromise).then(function(response) {
                        console.log('[SOCKET-EC] Relay success type=' + outgoingType);
                        window.postMessage({
                            type: 'socket_ec_response',
                            callbackId: callbackId,
                            targetRuntimeId: currentRuntimeId(),
                        ...
                        console.warn('[SOCKET-EC] Relay error type=' + outgoingType + ': ' + ((error && error.message) ? error.message : String(error)));
                        window.postMessage({
                            type: 'socket_ec_response',
                            callbackId: callbackId,
                            targetRuntimeId: currentRuntimeId(),
                ...
                function relayConnectOpen(data) {
                    ...
                    if (!runtimeAPI || typeof runtimeAPI.connect !== 'function') {
                        window.postMessage({
                            type: 'socket_ec_connect_disconnect',
                            portId: portId,
                            targetRuntimeId: currentRuntimeId(),
                    ...
                    console.log('[SOCKET-EC] Relay connect open id=' + portId + ' name=' + (connectInfo.name || '') + ' ext=' + (data.extensionId || '(none)'));
                    ...
                    port.onMessage.addListener(function(message) {
                        window.postMessage({
                            type: 'socket_ec_connect_message',
                            portId: portId,
                            targetRuntimeId: currentRuntimeId(),
                            message: message
                        }, '*');
                    ...
                    port.onDisconnect.addListener(function() {
                        ...
                        delete bridgePorts[portId];
                        console.log('[SOCKET-EC] Relay connect disconnect id=' + portId + (error ? (' error=' + error) : ''));
                        window.postMessage({
                            type: 'socket_ec_connect_disconnect',
                            portId: portId,
                            targetRuntimeId: currentRuntimeId(),
                ...
                window.addEventListener('message', function(event) {
                    if (event.source !== window) return;
                    if (!event.data || typeof event.data.type !== 'string') return;
                    if (event.data.type === 'socket_ec_request') {
                        if (!isTargetedMessage(event.data)) return;
                        relaySendMessage(event.data);
                        return;
                    }
                    if (event.data.type === 'socket_ec_connect_open') {
                        if (!isTargetedMessage(event.data)) return;
                        relayConnectOpen(event.data);
                        return;
                    }
                    if (event.data.type === 'socket_ec_connect_post') {
                        if (!isTargetedMessage(event.data)) return;
                        relayConnectPost(event.data);
                        return;
                    }
                    if (event.data.type === 'socket_ec_connect_close') {
                        if (!isTargetedMessage(event.data)) return;
            ...
            if existingBridgeJS != bridgeJS {
                ...
                Self.logger.info("Updated socket_bridge.js for externally_connectable bridge compatibility")
                changed = true
            }

            if existingBridgeIndex == nil {
                Self.logger.info("Created socket_bridge.js and registered in manifest for externally_connectable bridge")
            }
        }
        ...
        if isBitwarden {
            ...
            let bridgeExists = contentScripts.contains { entry in
                (entry["js"] as? [String])?.contains("socket_iframe_bridge.js") == true
            }
            if !bridgeExists {
                let iframeBridgeEntry: [String: Any] = [
                    "all_frames": true,
                    "js": ["socket_iframe_bridge.js"],
                    "matches": ["<all_urls>"],
                    "run_at": "document_start",
                    "world": "MAIN"
                ]
                ...
                manifest["content_scripts"] = contentScripts
                changed = true
                Self.logger.info("patchManifestForWebKit: added socket_iframe_bridge.js for Bitwarden iframe message forwarding")
            }

            // Write the bridge file
            let bridgeDir = manifestURL.deletingLastPathComponent()
            let bridgeFileURL = bridgeDir.appendingPathComponent("socket_iframe_bridge.js")
            let bridgeJS = """
            // Socket: Bitwarden iframe message bridge
            ...
            if existingBridgeJS != bridgeJS {
                try? bridgeJS.write(to: bridgeFileURL, atomically: true, encoding: .utf8)
                Self.logger.info("Updated socket_iframe_bridge.js for Bitwarden iframe message forwarding")
                changed = true
            }
        }
    ...
    private func getExtensionsDirectory() -> URL {
        ...
        return appSupport.appendingPathComponent("Socket").appendingPathComponent(
            "Extensions"
        )
    }
    ...
    func diagnoseExtensionState(for webView: WKWebView, url: URL) {
        ...
        for (extId, ctx) in extensionContexts {
            ...
            if let bgWV = bgWV {
                ...
                """) { result, error in
                    let logger = Logger(subsystem: "com.socket.browser", category: "Extensions")
                    if let json = result as? String {
...
class PopupUIDelegate: NSObject, WKUIDelegate, WKNavigationDelegate {
    private static let logger = Logger(subsystem: "com.socket.browser", category: "ExtensionPopup")
