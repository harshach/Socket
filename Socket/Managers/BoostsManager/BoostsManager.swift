...
class BoostsManager: ObservableObject {
    ...
    private let boostsKey = "socket_domain_boosts"
        ...
        return """
            (function() {
                if (typeof DarkReader !== 'undefined') {
                    ...
                    console.log('✅ [Socket Boost] Applied boost with DarkReader native font API:', {brightness: \(brightness), contrast: \(contrast), sepia: \(sepia), tintColor: '\(config.tintColor)', tintStrength: \(tintStrength), useFont: \(config.fontFamily != nil && !config.fontFamily!.isEmpty), fontFamily: '\(config.fontFamily ?? "none")'});
                } else {
                    console.error('❌ [Socket Boost] DarkReader not loaded');
                }
            })();
            """
    }
    ...
    private func getImmediateStyleInjection(for config: BoostConfig) -> String {
        ...
        return """
                            ...
                            style.id = 'socket-boost-font-immediate';
    ...
    private func getFontSettingsScript(for config: BoostConfig) -> String {
        ...
        if let fontFamily = config.fontFamily, !fontFamily.isEmpty {
            ...
            variableScripts.append("""
                document.documentElement.style.setProperty('--socket-boost-font-family', '\(escapedFont)', 'important');
            """)
            cssRules.append("font-family: var(--socket-boost-font-family, inherit) !important;")
        }
        
        if config.textTransform != "none" {
            variableScripts.append("""
                document.documentElement.style.setProperty('--socket-boost-text-transform', '\(config.textTransform)', 'important');
            """)
            cssRules.append("text-transform: var(--socket-boost-text-transform, inherit) !important;")
        }
        ...
        return """
            (function() {
                ...
                const style = document.createElement('style');
                style.id = 'socket-boost-font-settings';
    ...
    private func getZoomScript(for zoom: Int) -> String {
        ...
        return """
            (function() {
                const style = document.createElement('style');
                style.id = 'socket-boost-page-zoom';
    ...
    private func getCustomCodeScript(for config: BoostConfig) -> String {
        ...
        if !config.customCSS.isEmpty {
            ...
            scripts.append("""
                (function() {
                    const style = document.createElement('style');
                    style.id = 'socket-boost-custom-css';
    ...
    func disableBoost(in webView: WKWebView, completion: ((Bool) -> Void)? = nil) {
        let disableScript = """
            (function() {
                if (typeof DarkReader !== 'undefined') {
                    DarkReader.disable();
                    console.log('✅ [Socket Boost] DarkReader disabled');
                }
            })();
            """
    ...
    func injectCSS(_ css: String, into webView: WKWebView) {
        ...
        if css.isEmpty {
            script = """
                (function() {
                    const existingStyle = document.getElementById('socket-boost-custom-css');
                    if (existingStyle) {
                        existingStyle.remove();
                        console.log('✅ [Socket Boost] Custom CSS removed');
                    }
                })();
                """
        } else {
            ...
            script = """
                (function() {
                    ...
                    style.textContent = '\(escapedCSS)';
                    document.head.appendChild(style);
                    console.log('✅ [Socket Boost] Custom CSS injected');
                })();
                """
        }
    ...
    func applyPageZoom(_ zoom: Int, to webView: WKWebView) {
        ...
        let script = """
            (function() {
                const existingStyle = document.getElementById('socket-boost-page-zoom');
                if (existingStyle) {
                    existingStyle.remove();
                }
                ...
                style.textContent = 'html { zoom: \(zoomValue) !important; }';
                document.head.appendChild(style);
                console.log('✅ [Socket Boost] Page zoom set to \(zoom)%');
            })();
            """
