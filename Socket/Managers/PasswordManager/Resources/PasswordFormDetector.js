...
(function () {
    ...
    function ensureInlineIcon(passwordEl) {
        if (passwordEl.__socketIconInstalled) { return; }
        passwordEl.__socketIconInstalled = true;
        if (passwordEl.ownerDocument !== document) { return; } // shadow/other
        ...
        icon.addEventListener("click", function (e) {
            e.preventDefault();
            e.stopPropagation();
            var info = describeFieldsForFocus(passwordEl);
            requestAutofill(info, "icon");
        }, true);
        ...
        function updatePosition() {
            var rect = passwordEl.getBoundingClientRect();
            ...
            // Park ours to the LEFT of where a site-rendered show/hide-password
            // icon typically lives (~right-edge of the field). We reserve 36px
            // of clearance so both icons are visible. Sites without a native
            // eye icon just show ours a bit further inset, which is acceptable.
            var gutterRight = 36;
        ...
        var detachObserver = new MutationObserver(function () {
            if (!document.contains(passwordEl)) {
                ...
                passwordEl.__socketIconInstalled = false;
            } else {
                updatePosition();
            }
        });
    ...
    function scan() {
        ...
        for (var i = 0; i < pwFields.length; i++) {
            ...
            if (isTopFrame) { ensureInlineIcon(pwFields[i]); }
        }
    }
