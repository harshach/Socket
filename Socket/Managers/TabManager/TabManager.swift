...
/// a best-effort atomic save using a child ModelContext pattern.
    ...
    private static let log = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Socket", category: "TabPersistence")
...
class TabManager: ObservableObject {
    ...
    @Published public private(set) var hasCompletedInitialLoad: Bool = false
    ...
    private func attach(_ tab: Tab) {
        ...
        tab.socketSettings = socketSettings
    }
    ...
    func createSpace(
        ...
        // Always assign to a profile - prefer the explicit profile, then current profile, then the default profile.
        ...
        let resolvedGradient =
            (gradient.visuallyEquals(.default)
                ? (browserManager?.profileManager.profiles.first(where: { $0.id == profileId })?.gradient ?? gradient)
                : gradient)
        let space = Space(
            name: name,
            icon: icon,
    ...
    func moveSpaceUp(_ spaceId: UUID) {
        guard let currentIndex = spaces.firstIndex(where: { $0.id == spaceId }),
              currentIndex > 0 else { return }

        spaces.swapAt(currentIndex, currentIndex - 1)
        persistSnapshot()
    }

    func moveSpaceDown(_ spaceId: UUID) {
        guard let currentIndex = spaces.firstIndex(where: { $0.id == spaceId }),
              currentIndex < spaces.count - 1 else { return }
    ...
    private func provisionalTabName(for url: URL) -> String {
        ...
        case "about":
            return "New Tab"
        default:
            return url.lastPathComponent.isEmpty ? (url.host ?? "New Tab") : url.lastPathComponent
        }
    }
    ...
    private func ensureDefaultSpaceIfNeeded() -> Space {
        ...
        if spaces.isEmpty {
            ...
            let personalGradient =
                resolvedProfileId.flatMap { pid in
                    browserManager?.profileManager.profiles.first(where: { $0.id == pid })?.gradient
                } ?? .default
            let personal = Space(
                name: "Personal",
    ...
    func ensureUsableInitialState() {
        let resolvedSpace = ensureDefaultSpaceIfNeeded()
        let spacePinned = spacePinnedTabs[resolvedSpace.id] ?? []
        let regularTabs = tabsBySpace[resolvedSpace.id] ?? []
        let candidates = pinnedTabs + spacePinned + regularTabs

        var didMutate = false

        if currentSpace?.id != resolvedSpace.id {
            currentSpace = resolvedSpace
            didMutate = true
        }

        if let currentTab, contains(currentTab) {
            if didMutate {
                persistSnapshot()
            }
            return
        ...
        if let firstCandidate = candidates.first {
            if currentTab?.id != firstCandidate.id {
                currentTab = firstCandidate
                didMutate = true
            }
        } else {
            let defaultTab = createNewTab(url: "https://www.google.com", in: resolvedSpace)
        ...
        do {
            ...
            if spaces.isEmpty {
                let personalSpace = Space(
                    name: "Personal",
                    icon: "person.crop.circle",
                    gradient: browserManager?.currentProfile?.gradient ?? .default,
...
extension TabManager {
    ...
    func displayOrderedTabs(in space: Space) -> [Tab] {
        displayOrderedTabsWithDepth(in: space).map(\.tab)
    }

    func displayOrderedTabsWithDepth(in space: Space) -> [(tab: Tab, depth: Int)] {
        let regularTabs = tabs(in: space)
        let tabsById = Dictionary(uniqueKeysWithValues: regularTabs.map { ($0.id, $0) })
        let childrenByParent = Dictionary(grouping: regularTabs.filter { $0.parentTabId != nil }) { $0.parentTabId! }
        let roots = regularTabs.filter { tab in
            guard let parentId = tab.parentTabId else { return true }
            return tabsById[parentId] == nil
        }
        .sorted { $0.index < $1.index }

        var rows: [(tab: Tab, depth: Int)] = []

        func appendBranch(_ tab: Tab, depth: Int) {
            rows.append((tab: tab, depth: depth))
            let children = (childrenByParent[tab.id] ?? []).sorted { $0.index < $1.index }
            for child in children {
                appendBranch(child, depth: depth + 1)
            }
        }

        for root in roots {
            appendBranch(root, depth: 0)
        }

        return rows
    }

    func attachTab(_ childTab: Tab, asChildOf parentTab: Tab) {
        guard childTab.id != parentTab.id else { return }
        childTab.parentTabId = parentTab.id
        persistSnapshot()
    }
}
...
extension TabManager {
    ...
    private func trackRecentlyClosedTab(_ tab: Tab, spaceId: UUID?) {
        ...
        tabCopy.parentTabId = tab.parentTabId
