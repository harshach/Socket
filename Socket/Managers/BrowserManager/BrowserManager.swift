...
final class Persistence {
    ...
    nonisolated private static let log = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "Socket", category: "Persistence")
...
class BrowserManager: ObservableObject {
    ...
    var popupWindowManager = PopupWindowManager()
    ...
    init() {
        ...
        // settingsManager will be injected from SocketApp
        ...
        self.trackingProtectionManager.objectWillChange
            .sink { [weak self] _ in
        ...
        self.popupWindowManager.attach(browserManager: self)
    ...
    func maybeShowOAuthAssist(for url: URL, in tab: Tab) {
        ...
        guard socketSettings?.blockCrossSiteTracking == true, trackingProtectionManager.isEnabled else {
            return
        }
    ...
    func toggleAISidebar() {
        guard socketSettings?.showAIAssistant == true else { return }
        if let windowState = windowRegistry?.activeWindow {
    ...
    func toggleTopBarAddressView() {
        withAnimation(.easeInOut(duration: 0.2)) {
            socketSettings?.topBarAddressView.toggle()
        }
    }
    ...
    func createNewTab(
        ...
        if windowState.isIncognito, let profile = windowState.ephemeralProfile {
            let template = socketSettings?.resolvedSearchEngineTemplate ?? SearchProvider.google.queryTemplate
            let normalizedURL = normalizeURL(url, queryTemplate: template)
            guard let resolvedUrl = URL(string: normalizedURL) else { return nil }
    ...
    /// Create a tab intended for split view without replacing the window's current main tab selection.
    @discardableResult
    func createTabForSplit(
        in windowState: BrowserWindowState,
        url: String = "https://www.google.com"
    ) -> Tab? {
        // Split mode is not currently supported for ephemeral/incognito windows via the palette path.
        if windowState.isIncognito {
            return createNewTab(in: windowState, url: url)
        }
        ...
        return tabManager.createNewTab(url: url, in: targetSpace)
    }
    ...
    func showQuitDialog() {
        if self.socketSettings?.askBeforeQuit == true {
    ...
    private func showAppearanceEditor(
        initialGradient: SpaceGradient,
        title: String,
        subtitle: String,
        onCancel: @escaping () -> Void,
        onSave: @escaping (SpaceGradient) -> Void
    ) {
        let draft = GradientDraft(initialGradient)
    ...
    func showGradientEditor() {
        ...
        showAppearanceEditor(
            initialGradient: space.gradient,
            title: "Space Appearance",
            subtitle: space.name,
            ...
            onSave: { [weak self] updatedGradient in
                space.gradient = updatedGradient
                self?.gradientColorManager.endInteractivePreview()
                self?.gradientColorManager.transition(to: updatedGradient, duration: 0.35)
                self?.refreshGradientsForSpace(space, animate: true)
    ...
    func showProfileAppearanceEditor(for profile: Profile) {
        showAppearanceEditor(
            initialGradient: profile.gradient,
            title: "Profile Appearance",
            subtitle: profile.name,
            onCancel: { [weak self] in
                self?.gradientColorManager.endInteractivePreview()
                if let currentSpace = self?.tabManager.currentSpace {
                    self?.gradientColorManager.transition(to: currentSpace.gradient, duration: 0.25)
                    self?.refreshGradientsForSpace(currentSpace, animate: true)
                }
            },
            onSave: { [weak self] updatedGradient in
                guard let self else { return }
                self.profileManager.updateGradient(updatedGradient, for: profile.id)

                for space in self.tabManager.spaces where space.profileId == profile.id {
                    space.gradient = updatedGradient
                    self.refreshGradientsForSpace(space, animate: true)
                }

                if self.tabManager.currentSpace?.profileId == profile.id {
                    self.gradientColorManager.endInteractivePreview()
                    self.gradientColorManager.transition(to: updatedGradient, duration: 0.35)
                } else {
                    self.gradientColorManager.endInteractivePreview()
    ...
    func showProfileCreationDialog(in windowState: BrowserWindowState? = nil) {
        dialogManager.showDialog(
            ProfileCreationDialog(
                isNameAvailable: { [weak self] proposed in
                    guard let self else { return false }
                    let trimmed = proposed.trimmingCharacters(in: .whitespacesAndNewlines)
                    guard !trimmed.isEmpty else { return false }
                    return !self.profileManager.profiles.contains {
                        $0.name.caseInsensitiveCompare(trimmed) == .orderedSame
                    }
                },
                onCreate: { [weak self] name, icon in
                    guard let self else { return }
                    _ = self.createProfileAndActivate(
                        name: name,
                        icon: icon,
                        in: windowState
                    )
    ...
    func createProfileAndActivate(
        ...
        let created = profileManager.createProfile(name: trimmed, icon: safeIcon)

        if tabManager.spaceCount(for: created.id) == 0 {
            _ = tabManager.createSpace(
                name: "Personal",
                icon: safeIcon,
    ...
    func tabsForDisplay(in windowState: BrowserWindowState) -> [Tab] {
        ...
        let regularTabs = currentSpace.map { tabManager.displayOrderedTabs(in: $0) } ?? []
...
/// DEPRECATED: Use WebViewCoordinator.getWebView() directly via environment
    ...
    func reloadTabFromOriginAcrossWindows(_ tabId: UUID) {
        webViewCoordinator?.reloadTabFromOrigin(tabId)
    }
    ...
    func moveCurrentSpaceUp() {
        guard let activeSpaceId = windowRegistry?.activeWindow?.currentSpaceId else { return }
        tabManager.moveSpaceUp(activeSpaceId)
    }

    func moveCurrentSpaceDown() {
        guard let activeSpaceId = windowRegistry?.activeWindow?.currentSpaceId else { return }
        tabManager.moveSpaceDown(activeSpaceId)
    }

    func showCreateSpaceDialog() {
        dialogManager.showDialog(
            SpaceCreationDialog(
                onCreate: { [weak self] name, icon, selectedProfileId in
                    guard let self else { return }
                    let trimmedName = name.trimmingCharacters(in: .whitespacesAndNewlines)
                    let resolvedProfileId =
                        selectedProfileId
                        ?? self.windowRegistry?.activeWindow?.currentProfileId
                        ?? self.currentProfile?.id
                    let resolvedGradient =
                        resolvedProfileId.flatMap { id in
                            self.profileManager.profiles.first(where: { $0.id == id })?.gradient
                        }
                        ?? self.gradientColorManager.displayGradient
    ...
    func showEditCurrentSpaceDialog() {
        guard let activeWindow = windowRegistry?.activeWindow,
              let currentSpace = activeWindow.currentSpace else { return }

        dialogManager.showDialog(
            SpaceEditDialog(
                space: currentSpace,
                mode: .rename,
                onSave: { [weak self] name, icon, selectedProfileId in
                    guard let self else { return }
                    currentSpace.name = name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? currentSpace.name : name.trimmingCharacters(in: .whitespacesAndNewlines)
                    currentSpace.icon = icon.isEmpty ? currentSpace.icon : icon
                    if let profileId = selectedProfileId {
                        currentSpace.profileId = profileId
                        if let gradient = self.profileManager.profiles.first(where: { $0.id == profileId })?.gradient {
                            currentSpace.gradient = gradient
                        }
                    }
                    self.setActiveSpace(currentSpace, in: activeWindow)
                    self.tabManager.persistSnapshot()
                    self.dialogManager.closeDialog()
                },
                onCancel: { [weak self] in
    ...
    func showShortcutsSettings() {
        socketSettings?.currentSettingsTab = .shortcuts
        NSApp.sendAction(Selector(("showSettingsWindow:")), to: nil, from: nil)
    }

    func showExtensionsSettings() {
    ...
    func refreshShieldsLists() {
        Task { @MainActor [weak self] in
            await self?.trackingProtectionManager.refreshListsAndRecompile(forceRemoteUpdate: true)
        }
    }

    func toggleFocusMode() {
        guard let activeWindow = windowRegistry?.activeWindow else { return }
        toggleFocusMode(for: activeWindow)
    }

    func toggleFocusMode(for windowState: BrowserWindowState) {
        withAnimation(.easeInOut(duration: 0.2)) {
            windowState.isFocusModeEnabled.toggle()
    ...
    func toggleSplitMode() {
        guard let activeWindow = windowRegistry?.activeWindow else { return }
        toggleSplitMode(for: activeWindow)
    }

    func toggleSplitMode(for windowState: BrowserWindowState) {
        if splitManager.isSplit(for: windowState.id) {
            let keepSide = splitManager.activeSide(for: windowState.id) ?? .left
            splitManager.exitSplit(keep: keepSide, for: windowState.id)
        } else {
            windowState.commandPalette?.openInSplit()
        }
    }

    func sendCurrentPageToSplit() {
        guard let activeWindow = windowRegistry?.activeWindow,
              let currentTab = currentTab(for: activeWindow) else { return }

        if splitManager.isSplit(for: activeWindow.id) {
            splitManager.enterSplit(with: currentTab, placeOn: .right, in: activeWindow)
            return
        }

        let duplicate = tabManager.duplicateAsRegularForSplit(
            from: currentTab,
            anchor: currentTab,
            placeAfterAnchor: true
        )
        splitManager.enterSplit(with: duplicate, placeOn: .right, in: activeWindow)
    }

    func promoteSplitPageToMain() {
        guard let activeWindow = windowRegistry?.activeWindow else { return }
        promoteSplitPageToMain(in: activeWindow)
    }

    func promoteSplitPageToMain(in windowState: BrowserWindowState) {
        guard splitManager.isSplit(for: windowState.id),
              let rightId = splitManager.rightTabId(for: windowState.id),
              let rightTab = tabManager.allTabs().first(where: { $0.id == rightId }) else { return }

        selectTab(rightTab, in: windowState)
        splitManager.exitSplit(keep: .right, for: windowState.id)
    }

    func closeSplitPage() {
        guard let activeWindow = windowRegistry?.activeWindow else { return }
        closeSplitPage(in: activeWindow)
    }

    func closeSplitPage(in windowState: BrowserWindowState) {
        guard splitManager.isSplit(for: windowState.id) else { return }
        splitManager.closePane(.right, for: windowState.id)
    }

    func refreshSplitPage() {
        guard let activeWindow = windowRegistry?.activeWindow,
              let rightId = splitManager.rightTabId(for: activeWindow.id),
              let webView = getWebView(for: rightId, in: activeWindow.id) else { return }
        webView.reload()
    }

    func focusSplitScreen() {
        guard let activeWindow = windowRegistry?.activeWindow,
              let rightId = splitManager.rightTabId(for: activeWindow.id),
              let rightTab = tabManager.allTabs().first(where: { $0.id == rightId }) else { return }
        selectTab(rightTab, in: activeWindow)
    }

    func toggleSplitScreenFocus() {
        guard let activeWindow = windowRegistry?.activeWindow,
              splitManager.isSplit(for: activeWindow.id),
              let activeSide = splitManager.activeSide(for: activeWindow.id) else { return }

        let targetId =
            activeSide == .left
            ? splitManager.rightTabId(for: activeWindow.id)
            : splitManager.leftTabId(for: activeWindow.id)
        guard let targetId,
              let targetTab = tabManager.allTabs().first(where: { $0.id == targetId }) else { return }

        selectTab(targetTab, in: activeWindow)
    }

    func setInsertModeEnabled(_ enabled: Bool) {
        guard let activeWindow = windowRegistry?.activeWindow else { return }
        activeWindow.isInsertModeEnabled = enabled

        guard let currentTab = currentTab(for: activeWindow),
              let webView = getWebView(for: currentTab.id, in: activeWindow.id) else { return }
        ...
        if enabled {
            ...
            activeWindow.window?.makeFirstResponder(nil)
        }
    }

    func focusOutOfWebContent() {
    ...
    func createNewWindow() {
        ...
        let contentView = ContentView()
            ...
            .environment(\.socketSettings, socketSettings ?? SocketSettingsService())
    ...
    func createIncognitoWindow() {
        ...
        newWindow.contentView = NSHostingView(rootView: contentView)
        newWindow.title = "Incognito - Socket"
        newWindow.minSize = NSSize(width: 470, height: 382)
        newWindow.contentMinSize = NSSize(width: 470, height: 382)
        newWindow.center()
...
extension BrowserManager {
    ...
    /// Sets Socket as the default browser for HTTP and HTTPS schemes
