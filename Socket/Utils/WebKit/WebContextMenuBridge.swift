...
final class WebContextMenuBridge: NSObject, WKScriptMessageHandler {
    ...
    (function() {
        if (window.__socketContextMenuBridgeInstalled) { return; }
        window.__socketContextMenuBridgeInstalled = true;
        console.log('[Socket Context Menu] Bridge script installed');
        ...
        function capturePayload(event) {
            console.log('[Socket Context Menu] capturePayload called');
            try {
                var invocations = 0;
                var params = {};
                ...
                console.log('[Socket Context Menu] Payload prepared:', JSON.stringify(payload));
                
                if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.contextMenuPayload) {
                    console.log('[Socket Context Menu] Posting message to native');
                    window.webkit.messageHandlers.contextMenuPayload.postMessage(payload);
                } else {
                    console.error('[Socket Context Menu] messageHandler not available!');
                }
            } catch (error) {
                console.error('[Socket Context Menu] Context menu payload error', error);
            }
        }

        document.addEventListener('contextmenu', capturePayload, true);
        console.log('[Socket Context Menu] Event listener registered');
    })();
    """
