...
struct CommandPaletteView: View {
    ...
    private func handleReturn() {
        if let site = activeSiteSearch {
            ...
            openURLInCurrentMode(navigateURL)
            text = ""
            activeSiteSearch = nil
            selectedSuggestionIndex = -1
            commandPalette.close()
    ...
    private func selectSuggestion(_ suggestion: SearchManager.SearchSuggestion)
    {
        ...
        case .tab(let existingTab):
            if commandPalette.shouldOpenInSplit {
                browserManager.splitManager.enterSplit(
                    with: existingTab,
                    placeOn: .right,
                    in: windowState
                )
                print("Opened existing tab in split: \(existingTab.name)")
            } else {
                browserManager.selectTab(existingTab, in: windowState)
                print("Switched to existing tab: \(existingTab.name)")
            }
        case .history(let historyEntry):
            openURLInCurrentMode(historyEntry.url.absoluteString)
            print("Opened history URL in window \(windowState.id)")
        case .url, .search:
    ...
    private func openURLInCurrentMode(_ url: String) {
        if commandPalette.shouldNavigateCurrentTab,
           browserManager.currentTab(for: windowState) != nil
        {
            browserManager.currentTab(for: windowState)?.navigateToURL(url)
            return
        }

        if commandPalette.shouldOpenInSplit,
           let newTab = browserManager.createTabForSplit(in: windowState, url: url)
        {
            browserManager.splitManager.enterSplit(
                with: newTab,
                placeOn: .right,
