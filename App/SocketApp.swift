...
struct SocketApp: App {
        ...
        WindowGroup {
            TransitionView(showB: $settingsManager.didFinishOnboarding) {
                OnboardingView()
                    ...
                    .environment(\.socketSettings, settingsManager)
    ...
    private func setupApplicationLifecycle() {
        ...
        browserManager.socketSettings = settingsManager
        browserManager.tabManager.socketSettings = settingsManager
        ...
        // Respect the persisted ad/tracker blocking preference from onboarding
        // and Settings instead of forcibly disabling it on launch.
        ...
        windowRegistry.onWindowClose = {
            ...
            if let browserManager = browserManager {
                ...
                print(
                    "⚠️ [SocketApp] Window \(windowId) closed after BrowserManager deallocation - performed minimal cleanup"
                )
            }
        }
        ...
        // The first window can register before these callbacks are attached.
        // Retroactively initialize any already-registered windows so startup
        // never stays stranded in the empty "No Spaces" shell.
...
/// Configures the window appearance and behavior for Socket browser windows
///
/// This modifier:
