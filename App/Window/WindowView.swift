...
private struct FocusModeControls: View {
    @EnvironmentObject var browserManager: BrowserManager
    @Environment(BrowserWindowState.self) private var windowState
    ...
    private var focusButton: some View {
        Button {
            ...
            Label("Exit Focus", systemImage: "viewfinder.circle.fill")
                .labelStyle(.iconOnly)
    ...
    private var splitButton: some View {
        Button {
            ...
            Label("Split", systemImage: splitIsActive ? "rectangle.split.2x1.fill" : "rectangle.split.2x1")
                .labelStyle(.iconOnly)
                .font(.system(size: 13, weight: .semibold))
                .frame(width: 30, height: 30)
        }
        ...
        .background(
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .fill(Color.primary.opacity(splitIsActive ? 0.16 : 0.08))
        )
        .help(splitIsActive ? "Close Split Mode" : "Open Split Mode")
    }

    private var closeLeftButton: some View {
        Button {
            browserManager.promoteSplitPageToMain(in: windowState)
        } label: {
            Label("Close Left Panel", systemImage: "arrow.left.to.line")
                .labelStyle(.iconOnly)
                .font(.system(size: 13, weight: .semibold))
                .frame(width: 30, height: 30)
        }
    ...
    private var closeRightButton: some View {
        Button {
            browserManager.closeSplitPage(in: windowState)
        } label: {
            Label("Close Split Page", systemImage: "xmark")
                .labelStyle(.iconOnly)
                .font(.system(size: 13, weight: .semibold))
                .frame(width: 30, height: 30)
        }
